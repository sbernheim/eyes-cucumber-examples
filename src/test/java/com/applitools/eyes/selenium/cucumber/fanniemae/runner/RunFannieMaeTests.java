package com.applitools.eyes.selenium.cucumber.fanniemae.runner;
...
@CucumberOptions(
    features = {                // A comma-delimited list of paths to feature file dirs.
        "classpath:fanniemae"    // Defaults to this package if left blank or blank string.
    },
    glue = {    // A comma-delimited list of packages containing step definitions.
        "com.applitools.eyes.selenium.cucumber.hooks",
        "com.applitools.eyes.selenium.cucumber.fanniemae.steps"
    },
...
public class RunFannieMaeTests extends AbstractTestNGCucumberTests {
    private static final Logger log = LoggerFactory.getLogger(RunFannieMaeTests.class.getName());
    
    private static final String batchName = "Eyes Cucumber Demo: Fannie Mae";
    private static final String defaultAppName = "Fannie Mae Web";

    private EyesBatchManager batchManager;
    
    public RunFannieMaeTests() {
        this.batchManager = new EyesBatchManager();
    }
    
    @BeforeSuite
    public void openEyesBatch(ITestContext ctx) {
        String testSuite = ctx.getSuite().getName();
        log.info("Before: Suite for {}", testSuite);
        String appName = (testSuite.isBlank() || testSuite.startsWith("Default suite") || testSuite.startsWith("Surefire suite")) ?  defaultAppName : testSuite;
        batchManager.setAppName(appName);
        batchManager.setBatchName(batchName);
    ...
    @AfterSuite
    public void closeEyesBatch(ITestContext ctx) {
        String testSuite = ctx.getSuite().getName();
